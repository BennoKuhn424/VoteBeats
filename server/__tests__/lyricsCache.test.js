...
describe('lyricsCache — basic set/get', () => {
  ...
  test('distinguishes by venue extra-words list', () => {
    cache.set('a1', ['en'], ['hate'], { hitCount: 1, lyricsFound: true });
    cache.set('a1', ['en'], ['drugs'], { hitCount: 4, lyricsFound: true });
    expect(cache.get('a1', ['en'], ['hate'])).toEqual({ hitCount: 1, lyricsFound: true });
    expect(cache.get('a1', ['en'], ['drugs'])).toEqual({ hitCount: 4, lyricsFound: true });
  });
...
describe('lyricsCache — key namespacing', () => {
  test('empty languages produces an empty language segment', () => {
    expect(cache.makeKey('a1', [], [])).toBe('a1||');
    expect(cache.makeKey('a1', undefined, undefined)).toBe('a1||');
  });
  ...
  test('extras are normalised, case-folded and sorted', () => {
    expect(cache.makeKey('a1', ['en'], ['Hate', 'drugs'])).toBe('a1|en|drugs,hate');
    expect(cache.makeKey('a1', ['en'], ['drugs', 'HATE'])).toBe('a1|en|drugs,hate');
  });
});
